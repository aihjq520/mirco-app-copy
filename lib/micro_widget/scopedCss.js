 ...
 * @param {string} pageName 子应用当前页面名称
 */
export default function scopedCSS (styleElement, pageName) {
    // 前缀
    const prefix = `.micro-app-${pageName}`
    ...
    if (styleElement.textContent) {
        ...
        const sheet = templateStyle.sheet ? templateStyle.sheet.cssRules : []
        styleElement.textContent = scopedRule(Array.from(sheet), prefix)
        ...
        const observer = new MutationObserver(function () {
            ...
            observer.disconnect()
            // 格式化规则，并将格式化后的规则赋值给style元素
            const sheet = styleElement.sheet ? templateStyle.sheet.cssRules : []
            styleElement.textContent = scopedRule(Array.from(sheet), prefix)
